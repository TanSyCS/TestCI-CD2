...
route.post('/saveDish', async (req, res) => {
    ...
    req.body.photo = "../image/img/"+imageName;
    const data = await dish.create(req.body)

    photo.mv("/Users/macbookpro/Downloads/BKFood-main/public/image/img/" + imageName);
    if (data) {
...
route.post("/admin/saveEditDish/:id", async (req, res) => {
    ...
    if (req.session.loginUser) {
        ...
        if (req.files == null)
            ...
            try {
                //edited
                const path = '/Users/macbookpro/Downloads/BKFood-main/public/image/img/' + req.body.tempImage;
                const modifiedPath = path.replace('../image/img/', ''); // Loại bỏ phần trùng
